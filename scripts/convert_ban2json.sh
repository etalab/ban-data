...
CREATE TABLE IF NOT EXISTS ban_temp (
	id TEXT,
	nom_voie TEXT, 
	id_fantoir TEXT, 
	numero TEXT, 
...
select format('{\"id\":\"%s-%s\",\"type\":\"%s\",\"name\":\"%s\",\"postcode\":\"%s\",\"lon\":%s,\"lat\": %s,\"city\":\"%s\",\"departement\":\"%s\",\"region\":\"%s\",\"importance\":%s,\"housenumbers\":{%s}}',code_insee, id_fantoir, type, nom_voie, code_post, lat, lon, nom_commune, nom_dept, nom_region, importance, housenumbers)
from
(select code_insee,
md5(format('f=%s,n=%s,l=%s,a=%s,p=%s',id_fantoir,nom_voie,nom_ld,alias,code_post)) id_fantoir,
...
string_agg(format('\"%s\":{\"lon\":%s,\"lat\":%s}',trim(numero||' '||rep),round(lon::numeric,6)::text,round(lat::numeric,6)::text),',' order by numero||rep) as housenumbers
from ban_temp b
