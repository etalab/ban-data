...
psql -c "
drop table if exists poste_cp;
create table poste_cp (insee text, commune text, cp text, libelle text);
\copy poste_cp from code_postaux_v201410_corr.csv with (format csv, header true, encoding 'iso8859-1', delimiter ';');
