...
echo "-- nombre de nom_voie vides ou nuls sans nom_ld (regroupés par département)"
...
echo "-- nom_voie contient nom_ld"
psql -c "\copy (select b.code_insee, b.id, 'nom_voie+nom_ld',b.nom_voie,'nom_voie contient nom_ld' from ban_temp b join ban_temp c on (c.id=b.id) where b.nom_voie !='' and c.nom_ld !='' and lower(unaccent(b.nom_voie)) like '%' || lower(unaccent(c.nom_ld)) || '%') to temp with (format csv, header false)"; cat temp >> erreurs.csv

echo "-- aucun nom ni code fantoir"
...
echo "-- nombre de nom_voie avec '/' (regroupés par département)"
...
echo "-- nombre de nom_voie avec '/' et répétitions (regroupés par département)"
...
echo "-- nombre de nom_voie différents avec id_fantoir identique (regroupés par département)"
...
echo "-- exemples de nom_voie différents avec id_fantoir identique"
...
echo "-- vérification erreurs courantes d'accentuation"
...
echo "-- vérification chiffres romains en minuscule"
...
echo "-- vérification de présence d'abbréviations résiduelles"
...
echo "-- vérification de présence d'abbréviations doublées"
#psql -P pager -c "select nom_voie, count(*) from ban_temp where nom_voie ~ '(^| )(ancien.* anc|route .* (route|rte)|chemin .*chem(in|)|grand.*(gde|grande))( |$)' and NOT nom_voie ~ '/' group by 1 order by 2 desc;"
...
echo "-- noms très longs"
#psql -P pager -c "select length(nom_voie) as longueur, nom_voie, code_insee, id_fantoir from ban_temp where length(nom_voie)>60 group by 1,2,3,4 order by 1 desc limit 50;"

echo "-- noms comportant des parenthèses"
#psql -P pager -c "select nom_voie, left(string_agg(distinct(code_insee),','),60) as exemple from ban_temp where nom_voie ~ '\(' or nom_voie ~ '\)' group by 1 order by 1;"
...
echo "-- noms comportant des tirets"
#psql -P pager -c "select nom_voie, left(string_agg(distinct(code_insee),','),60) as exemple from ban_temp where nom_voie ~ ' -' or nom_voie ~ '- ' or nom_voie ~ '--' group by 1 order by 1;"
...
echo "-- noms comportant des caractères étranges"
#psql -P pager -c "select nom_voie, left(string_agg(distinct(code_insee),','),60) as exemple from ban_temp where lower(unaccent(nom_voie)) !~ '[a-z0-9\-\/\(\)]' group by 1 order by 1;"
...
echo "-- nom_voie identique nom_ld"
...
echo "-- nom_voie et alias identiques"
...
echo "-- nom_ld et alias identiques"
...
echo "-- nom_ld avec premier mot doublé"
...
echo "-- nom_voie vide, nom_ld present et FANTOIR indique LD"
...
echo "-- nom_ld absence probable d'apostrophe apres D,L,QU,PRESQU"
...
echo "-- nom_voie commence par 'Enceinte' et nom_ld par 'EN '"
...
echo "-- nom_voie commence par 'sentier' et nom_afnor par 'SENTE '"
...
echo "-- test cohérence type voie entre nom_voie et nom_afnor"
