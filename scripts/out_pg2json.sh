...
select format('{\"id\":\"%s\",\"type\":\"%s\",\"name\":%s %s,\"postcode\":\"%s\",\"citycode\": %s,\"zonecode\":[\"%s\",\"%s\"],\"lon\":%s,\"lat\": %s,\"x\":%s,\"y\":%s,\"city\":\"%s\",\"context\":\"%s\",\"importance\":%s,\"housenumbers\":{%s}}',
  fantoir,
  type,
...
group by 1,2,4,alias, insee_2016, insee_2015, dep, reg
order by 1,2,3) as d;
"
