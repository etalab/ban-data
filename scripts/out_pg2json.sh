...
case when cn.insee is not null and lower(unaccent(nom_commune)) != lower(unaccent(g2.nom)) then format('(%s)',g2.nom) else '' end as ancienne_commune
from ban_$1 b
...
|| '\",\"type\": \"' || 'municipality' /* CASE WHEN population<1 THEN 'village' WHEN population<'10' THEN 'town' ELSE 'city' END */
|| '\",\"name\": \"' || g.nom
|| '\",\"postcode\": \"' || cp.cp
|| '\",\"citycode\": \"' || g.insee
