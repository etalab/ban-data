...
with u as (select * from abbrev where txt_long != txt_court ORDER BY length(txt_long) DESC) update dgfip_fantoir set lib_court=regexp_replace(lib_court,'(^| )'||txt_long||'( |$)','\1'||txt_court||'\2','g') from u where lib_court  ~ ('(^| )'||txt_long||'( |$)') ;

create index dgfip_fantoir_insee on dgfip_fantoir (code_insee,id_voie) with (fillfactor=100);
cluster dgfip_fantoir using dgfip_fantoir_insee;
"
