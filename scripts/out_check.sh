...
psql -c "\copy errors from erreurs.csv with (format csv, header true);"

echo "\n-- nombre d'adresse avec anomalie (groupé par département)\n"
psql -c "\copy (select e.dept, nb_err, nb_adr, round(100*nb_err/nb_adr,1) as pct_err from (select left(insee,2) as dept, count(distinct(id)) as nb_err from errors group by 1) as e join (select left(code_insee,2) as dept, count(*) as nb_adr from ban_temp group by 1) as a on (e.dept=a.dept) order by 1) to out_check_par_dept.csv with (format csv, header true);"
