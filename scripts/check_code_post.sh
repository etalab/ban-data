...
echo "-- code_post est vide (groupé par département)"
...
echo "-- code_post ne figurant pas dans la base officielle"
sql2csv --db "$DB" -H --query "select code_insee, id, 'code_post', code_post, 'code_post absent de la base officielle: '||cp from ban_temp b join (select insee, string_agg(cp,' ') as cp from poste_cp group by 1) as c on (code_insee=insee and not cp ~ code_post)" >> erreurs.csv
